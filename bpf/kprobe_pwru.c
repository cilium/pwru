...
set_output(void *ctx, struct sk_buff *skb, struct event_t *event) {
	...
	if (cfg->output_skb_metadata) {
		u64 skb_metadata[4] = {};
		set_skb_metadata(skb, skb_metadata);
		__builtin_memcpy(event->skb_metadata, skb_metadata, sizeof(skb_metadata));
	}
}
...
get_addr(void *ctx, const bool is_kprobe) {
	// Check also if it is equal to the standard enum number to avoid issues on older kernels https://github.com/torvalds/linux/commit/8a76145a2ec2a81dfe34d7ac42e8c242f095e8c8
	if (bpf_core_enum_value_exists(enum bpf_func_id, BPF_FUNC_get_func_ip) && 
		(bpf_core_enum_value(enum bpf_func_id, BPF_FUNC_get_func_ip) == BPF_FUNC_get_func_ip)) {
		/* endbr has been handled in helper. */
		return bpf_get_func_ip(ctx);
	}

	u64 ip = is_kprobe ? PT_REGS_IP((struct pt_regs *) ctx) : get_func_ip();
#ifdef bpf_target_x86
	ip -= ENDBR_INSN_SIZE;
