...
func loadExtInfos(r io.ReaderAt, bo binary.ByteOrder, strings stringTable) (*extInfo, error) {
	...
	buf := pkg.NewBufferedSectionReader(r, extHeader.funcInfoStart(), int64(extHeader.FuncInfoLen))
	funcInfos, err := parseFuncInfos(buf, bo, strings)
	if err != nil {
		return nil, fmt.Errorf("parsing BTF function info: %w", err)
	}

	buf = pkg.NewBufferedSectionReader(r, extHeader.lineInfoStart(), int64(extHeader.LineInfoLen))
	...
	if coreHeader != nil && coreHeader.CoreReloOff > 0 && coreHeader.CoreReloLen > 0 {
		buf = pkg.NewBufferedSectionReader(r, extHeader.coreReloStart(coreHeader), int64(coreHeader.CoreReloLen))
...
func (fi FuncInfo) Marshal(w io.Writer, offset uint64) error {
	...
	return binary.Write(w, pkg.NativeEndian, fi)
}
...
func (li LineInfo) Marshal(w io.Writer, offset uint64) error {
	...
	return binary.Write(w, pkg.NativeEndian, li)
}
