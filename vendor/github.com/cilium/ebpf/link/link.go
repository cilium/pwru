...
func AttachRawLink(opts RawLinkOptions) (*RawLink, error) {
	...
	if opts.Target < 0 {
		return nil, fmt.Errorf("invalid target: %s", pkg.ErrClosedFd)
	}

	progFd := opts.Program.FD()
	if progFd < 0 {
		return nil, fmt.Errorf("invalid program: %s", pkg.ErrClosedFd)
	}
...
func (l *RawLink) UpdateArgs(opts RawLinkUpdateOptions) error {
	newFd := opts.New.FD()
	if newFd < 0 {
		return fmt.Errorf("invalid program: %s", pkg.ErrClosedFd)
	}
	...
	if opts.Old != nil {
		oldFd = opts.Old.FD()
		if oldFd < 0 {
			return fmt.Errorf("invalid replacement program: %s", pkg.ErrClosedFd)
		}
	}
...
func (l *RawLink) Info() (*RawLinkInfo, error) {
	...
	err := pkg.BPFObjGetInfoByFD(l.fd, unsafe.Pointer(&info), unsafe.Sizeof(info))
	if err != nil {
